...
namespace Windows.Privilege.Application
{
    public class RoleService : BaseService, IRoleService
    {
        ...
        public RoleService(PrivilegeDbContext db,IMapper mapper)
        {
            _mapper = mapper;
            _db = db;
        }
        ...
        public async Task<List<RoleResponse>> GetUserRoles(int userId)
        {
            using (_db)
            {
                var roles = await (from a in _db.Role
                                   join b in _db.Role_Relation on a.Id equals b.RoleId
                                   where b.RelationId == userId
        ...
        public async Task<List<RoleResponse>> GetOrganizationRoles(int organizationId)
        {
            using (_db)
            {
                var roles = await (from a in _db.Role
                                   join b in _db.Role_Relation on a.Id equals b.RoleId
                                   where b.RelationId == organizationId
                                   select a).ToListAsync();
                return _mapper.Map<List<RoleResponse>>(roles);
            }
        }
        ...
        public async Task Save(RoleModifyRequest info)
        {
            ...
            //    {
            //        Domain.Privilege privilege = new Domain.Privilege(MasterEnum.Role, info.Id, AccessEnum.Module, id);
            //        await _db.AddAsync(privilege);
            //    }
            //    foreach (int id in info.OperateIds)
            //    {
            //        Domain.Privilege privilege = new Domain.Privilege(MasterEnum.Role, info.Id, AccessEnum.Operate, id);
            //        await _db.AddAsync(privilege);
            //    }
